...
namespace UsefulThings
{
    ...
    {
        /// <summary>
        /// Action to perform for visual feedback while dragging and dropping.
        /// </summary>
        public static Action GiveFeedbackAction = null;
        ...
        /// <summary>
        /// KFreon: My little constructor to make my life easier.
        /// Sets up a "copy" action to copy a file from C# to Windows somewhere.
        /// </summary>
        /// <param name="fileName">Name of file. NAME ONLY. No path.</param>
        /// <param name="data">File data to copy out.</param>
        /// <param name="feedbackAction">Action for providing visual feedback during drop drag.</param>
        public VirtualFileDataObject(string fileName, Stream data, Action feedbackAction) :
            this(feedbackAction, new FileDescriptor[]
            {
                new FileDescriptor
                {
                    Name = fileName,
                    ...
                    StreamContents = stream =>
                    {
                        ...
                        stream.ReadFrom(data, data.Length);
                        data.Seek(pos, SeekOrigin.Begin);  // Restore original position.
                    }
                }
            })
        {
        ...
        /// <param name="data">File data to copy out.</param>
        /// <param name="feedbackAction">Action for providing visual feedback during drop drag.</param>
        public VirtualFileDataObject(string fileName, byte[] data, Action feedbackAction) :
            this(feedbackAction, new FileDescriptor[]
                {
                    new FileDescriptor
                    {
                        ...
                        Length = data.Length,
                        ChangeTimeUtc = DateTime.Now,
                        StreamContents = stream => stream.Write(data, 0, data.Length)
                    }
                })
        {
        }
        ...
        /// Sets up a "copy" action to copy a file from C# to Windows somewhere.
        /// </summary>
        /// <param name="files">Files to copy out.</param>
        /// <param name="feedbackAction">Action for providing visual feedback during drop drag.</param>
        public VirtualFileDataObject(Action feedbackAction, params FileDescriptor[] files) :
            this()
        {
            SetData(files);
            GiveFeedbackAction = feedbackAction;
        }
