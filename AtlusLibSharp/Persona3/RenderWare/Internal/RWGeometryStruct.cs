...
namespace AtlusLibSharp.Persona3.RenderWare
{
    ...
    internal class RWGeometryStruct : RWNode
    {
        ...
        {
            get { return _geoFlags.HasFlagFast(RWGeometryFlags.HasVertices); }
        }

        public bool HasNormals
        {
            get { return _geoFlags.HasFlagFast(RWGeometryFlags.HasNormals); }
        }

        public bool HasColors
        {
            get { return _geoFlags.HasFlagFast(RWGeometryFlags.HasColors); }
        }

        public bool HasTexCoords
        {
            get { return _geoFlags.HasFlagFast(RWGeometryFlags.HasTexCoord1); }
        }
        ...
        : base(header)
        {
            ...
            if (_geoFlags.HasFlagFast(RWGeometryFlags.HasColors))
            {
                _clrArray = reader.ReadColorArray(numVerts);
            }

            if (_geoFlags.HasFlagFast(RWGeometryFlags.HasTexCoord1) ||
               (_geoFlags.HasFlagFast(RWGeometryFlags.HasTexCoord2)))
            {
            ...
            if (_geoFlags.HasFlagFast(RWGeometryFlags.HasVertices))
            {
                _posArray = reader.ReadVector3Array(numVerts);
            }

            if (_geoFlags.HasFlagFast(RWGeometryFlags.HasNormals))
            {
