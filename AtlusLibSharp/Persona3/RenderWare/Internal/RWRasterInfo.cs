namespace AtlusLibSharp.Persona3.RenderWare
{
    ...
    internal class RWRasterInfo : RWNode
    {
        ...
        /// <summary>
        /// Initialize a new instance of <see cref="RWRasterInfo"/> with properties set using a given width, height and <see cref="PS2.Graphics.PixelFormat"/>.
        /// </summary>
        /// <param name="width"></param>
        /// <param name="height"></param>
        /// <param name="pixelFormat">PS2 pixel format of the given texture data.</param>
        public RWRasterInfo(int width, int height, PixelFormat pixelFormat)
            : base(RWType.Struct)
        {
        ...
        /// <param name="pixelFormat">PS2 pixel format of the given texture data.</param>
        private void CreateRasterInfo(int width, int height, PixelFormat pixelFormat)
        {
            _width = width;
            _height = height;
            _depth = PixelFormatHelper.GetPixelFormatDepth(pixelFormat);
            ...
            {
                _tex1.MaxMipLevel = 7;
                _tex1.MipMinFilter = FilterMode.None;

                if (texSize <= 4096)
                {
                    _tex1.MipMaxFilter = FilterMode.None;
                }
                else
                {
                    _tex1.MipMaxFilter = FilterMode.Nearest;
                }
            }
            ...
            _texelDataLength = (uint)PixelFormatHelper.GetTexelDataSize(pixelFormat, width, height);

            // Add image header size to the length if there is one
            if (_rasterFormat.HasFlagFast(RWRasterFormats.HasHeaders))
                _texelDataLength += StandardImageHeader.Size;
            ...
            if (_depth == 4 || _depth == 8)
            {
                ...
                // Add image header size to the length if there is one
                if (_rasterFormat.HasFlagFast(RWRasterFormats.HasHeaders))
                    _paletteDataLength += StandardImageHeader.Size;
            }
